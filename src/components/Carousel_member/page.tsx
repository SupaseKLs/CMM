    ...
    <div className="grid grid-cols-3 gap-10">
      {members.map((member, index) => (
        <div key={index} className="relative group overflow-hidden rounded-lg">
          {/* รูปภาพ */}
          <img className="w-full transition duration-300 group-hover:blur-md" 
          ...
          {/* ข้อความซ้ายบน */}
          <p className="absolute top-14 left-14 text-white text-4xl font-semibold opacity-0 group-hover:opacity-100 transition-all duration-500">
            อาจารย์
          </p>

          {/* ข้อความซ้ายล่าง */}
          <p className="absolute bottom-14 left-14 text-white text-6xl font-semibold opacity-0 group-hover:opacity-100 transition-all duration-500">
            {member.name}
          </p>
        </div>
      ))}
